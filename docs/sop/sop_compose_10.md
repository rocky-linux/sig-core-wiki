---
title: 'SOP: Compose and Repo Sync for Rocky Linux 10'
---

This SOP covers how the Rocky Linux Release Engineering Team handles composes and repository syncs for Rocky Linux 10. It contains information of the scripts that are utilized and in what order, depending on the use case.
...
* `/mnt/compose` -> Compose data, symlink to /mnt/koji/compose
* `/mnt/repos-staging` -> Staging
* `/mnt/repos-production` -> Production

### Pungi

Each repository or set of repositories are controlled by various pungi configurations. For example, `rocky.conf` will control the absolute base of Rocky Linux 10, which imports other git repository data as well as accompanying json or other configuration files.
...
When these scripts are ran, they generate an appropriate directory under `/mnt/compose/X` with a directory and an accompanying symlink. For example. If an update to `Rocky` was made using `updates-10.sh`, the below would be made:

```
drwxr-xr-x. 5 root  root  6144 Jul 21 17:44 Rocky-10-updates-20250525.1
lrwxrwxrwx. 1 root  root    26 Jul 21 18:26 latest-Rocky-10 -> Rocky-10-updates-20250525.1
```

This setup also allows pungi to reuse previous package set data to reduce the time it takes to build a compose. Typically during a new minor release, all composes should be ran so they can be properly combined. Example of a typical order if releasing 10.X:

```
produce-10.sh
updates-10-devel.sh
updates-10-extras.sh
...
```
pushd /mnt/repos-staging/mirror/pub/rocky/10.X
python3 /usr/local/bin/apollo_tree -p $(pwd) -n 'Rocky Linux 8 $arch' -i Live -i Minimal -i devel -i extras -i images -i isos -i live -i metadata -i Devel -i plus -i nfv
popd
